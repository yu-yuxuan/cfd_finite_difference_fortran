...
contains
  ...
  subroutine two_d_BG_FTBS(nu,dt,t,dx,x,dy,y,u0,u,v0,v)


    implicit none
    real(kind=8),intent(in)::nu,dt,dx,t,x,dy,y
       ...
       Do jspace=2,nint(y/dy)
          ...
          Do ispace=2,nint(x/dx)
             ...
             v(jspace, ispace)=vt_last(jspace,ispace)&
                  ...
                  +nu*dt/(dx**2)*(vt_last(jspace,ispace+1)-2*vt_last(jspace,ispace)+vt_last(jspace,ispace-1))&
                  +nu*dt/(dy**2)*(vt_last(jspace+1,ispace)-2*vt_last(jspace,ispace)+vt_last(jspace-1,ispace))
