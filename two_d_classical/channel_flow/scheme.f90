...
!============peridoic condition============
  ...
  subroutine two_d_laplace_p(dx,x,x_value,dy,y,y_value,p0,p,b,epsilon,iepsilon)
    ...
    do
       ...
       do jspace=2,nint(y/dy)
          ...
          p(jspace,1)=((ptemp_J(jspace,2)+ptemp_J(jspace,nint(x/dx)))/(dx**2.d0)&
               +(ptemp_J(jspace+1,1)+ptemp_J(jspace-1,1))/(dy**2.d0) -b(jspace,1))&
               /(2.d0/(dx**2.d0)+2.d0/(dy**2.d0))
  ...
  !============two dimension channel function============

  subroutine two_d_uv_channel(nu,rho,dt,t,dx,x,dy,y,u0,u,v0,v,p)
    ...
    Do jspace=2,nint(y/dy)
       ...
       Do ispace=2,nint(x/dx)
          ...
          u(jspace, ispace)=ut_last(jspace,ispace)&
               ...
               +1.d0*dt   !This is a source added, it will be delleted depending what condtions you are
    ...
    Do jspace=2,nint(y/dy)
       ...
       u(jspace, 1)=ut_last(jspace,1)&
            -ut_last(jspace,1)*dt/dx*(ut_last(jspace,1)-ut_last(jspace,nint(x/dx))) &
            -vt_last(jspace,1)*dt/dy*(ut_last(jspace,1)-ut_last(jspace-1,1)) &
            -dt/rho*(p(jspace,2)-p(jspace,nint(x/dx)))/2.d0/dx &
            +nu*dt/(dx**2.d0)*(ut_last(jspace,2)-2.d0*ut_last(jspace,1)+ut_last(jspace,nint(x/dx)))&
            +nu*dt/(dy**2.d0)*(ut_last(jspace+1,1)-2.d0*ut_last(jspace,1)+ut_last(jspace-1,1))&
            +1.d0*dt   !This is a source added, it will be delleted depending what condtions you are
    ...
    Do jspace=2,nint(y/dy)
       ...
       v(jspace, 1)=vt_last(jspace,1)&
            -ut_last(jspace,1)*dt/dx*(vt_last(jspace,1)-vt_last(jspace,nint(x/dx))) &
            -vt_last(jspace,1)*dt/dy*(vt_last(jspace,1)-vt_last(jspace-1,1)) &
            -dt/rho*(p(jspace+1,1)-p(jspace-1,1))/2.d0/dy &
            +nu*dt/(dx**2.d0)*(vt_last(jspace,2)-2.d0*vt_last(jspace,1)+vt_last(jspace,nint(x/dx)))&
            +nu*dt/(dy**2.d0)*(vt_last(jspace+1,1)-2.d0*vt_last(jspace,1)+vt_last(jspace-1,1))
    ...
    ! u(3,2)=7
    ! print *, u(:,1)-2*u(:,2)
  end subroutine two_d_uv_channel
